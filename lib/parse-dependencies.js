...
export default function parseDependencies(metadata) {
	...
	let deps = [...unique].filter(dep => !dep.startsWith('!'));
	deps.sort();
	return deps;
...
function getDependency({ link, text }) {
	...
	// Check first if we have a raw url match.
	...
	if (hostname.includes('simtropolis')) {
		if (url.pathname.match(/\/(forums|profile|tags|topic|omnibus)\//)) return null;
		if (url.pathname.match(/messageview\.cfm/)) return null;
	}
	...
	// Exclude stuff like wikipedia.
	if (hostname.includes('wikipedia.org')) return null;
	...
	if (hostname.includes('simtropolis.com')) {
		...
		if (url.pathname.includes('bldgprop')) return 't-wrecks:maxis-prop-names-and-query-fix';
	...
	// Ignore links to PayPal or Patreon.
	if (hostname.includes('paypal')) return null;
	if (hostname.includes('patreon')) return null;
	if (hostname.includes('cgtrader')) return null;
	if (hostname.includes('sketchup')) return null;
