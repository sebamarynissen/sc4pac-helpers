...
class Patcher {
	...
	async patch(patterns, opts = {}) {
		...
		for (let file of files) {
			...
			for (let season of seasons) {
				...
				this.outputs[key] ??= {
					fullPath: path.join(output, season, filename),
					filename,
					season,
					dbpf: new DBPF(),
				};
		...
		for (let { fullPath, dbpf } of Object.values(this.outputs)) {
	...
	createPatch({ exemplar, entry, pkg, seasons }) {
		...
		const families = new Set(models
			...
			.flat());
		if (families.size === 0) {
			console.warn(`Unable to find a family for ${type} exemplar ${chalk.green(name)} (${chalk.yellow(tgi)})`);
			return;
		}
	...
	patchRKT1({ exemplar, model }) {
		...
		// const rktm = exemplar.get('ResourceKeyType1xm');
		// if (rktm) {
		// 	props.push([Prop.ResourceKeyType1xm, [FileType.S3D, ...model]]);
		// }
		return props;
	}
