...
export default class TreeDatabase extends Array {
	...
	findSeasonModelFromFamily(family, season) {
		...
		// Depending on the season we're requesting, the actual season we 
		// return can depend on which models we have or not.
		const fallback = ({
			spring: ['summer', 'evergreen'],
			summer: ['evergreen', 'spring'],
			fall: ['winter', 'evergreen', 'summer'],
			winter: ['fall', 'evergreen', 'summer'],
			snow: ['winter', 'fall', 'evergreen', 'summer'],
		})[season];
		const found = [season, ...fallback].find(season => {
			return Object.hasOwn(models, season);
		});
		return models[found]?.model ?? null;
...
function lookup(map, order) {
	const season = priority.find(season => {
		return Object.hasOwn(map, season);
	});
	return map[season];
}
